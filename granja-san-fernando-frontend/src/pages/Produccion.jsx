...
function Produccion() {
  ...
  const [nuevaGalera, setNuevaGalera] = useState({ nombre: '', ubicacion: '', capacidad: '' });
  ...
  const handleCrearGalera = async (e) => {
    ...
    try {
      ...
      setNuevaGalera({ nombre: '', ubicacion: '', capacidad: '' });
  ...
  return (
    <>
      ...
      <section className="card">
        ...
        {mostrarNuevaGalera && (
          <form onSubmit={handleCrearGalera} style={{ display: 'flex', gap: '10px', marginBottom: '16px', flexWrap: 'wrap', alignItems: 'flex-end' }}>
            <div className="field">
            ...
            <button type="submit" className="btn">Guardar galera</button>
          </form>
        )}
        ...
        {lotes.length === 0 ? (
          ...
          <table>
            ...
            <tbody>
              {lotes.map((l) => (
                <tr key={l.id_lote}>
                  <td>{l.galera_nombre}</td>
                  <td>{l.fecha_ingreso?.slice(0, 10)}</td>
                  <td>{l.aves_recibidas}</td>
                  <td>{l.aves_activas}</td>
                  <td><span className={`tag ${l.estado === 'activo' ? 'ok' : 'low'}`}>{l.estado}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
            ...
            <tbody>
              {postura.map((p) => (
                <tr key={p.id_postura}>
                  ...
                  <td>{p.cantidad_huevos}</td>
                  <td>{p.tasa_postura}%</td>
                </tr>
              ))}
            </tbody>
          </table>
